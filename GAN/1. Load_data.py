...
def get_technical_indicators(data):
    # Create 7 and 21 days Moving Average 移动平均
    ...
    # Create Bollinger Bands 布林线
    data['20SD'] = data.iloc[:, 4].rolling(20).std()
    data['upper_band'] = data['MA21'] + (data['20SD'] * 2)
    data['lower_band'] = data['MA21'] - (data['20SD'] * 2)

    # Create Exponential moving average 指数移动平均
    data['EMA'] = data.iloc[:,4].ewm(com=0.5).mean()

    # Create LogMomentum 对数动量指标
...
#For doing the fourier傅里叶变换
dataset = T_df.iloc[20:,:].reset_index(drop=True)

#Getting the Fourier transform features得到傅里叶转化特征
